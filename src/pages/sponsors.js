...
class Sponsors extends React.Component {
  render() {
    ...
    const listSponsorsProfessional = this.props.data.sponsorsProfessionalLogos.edges.map(
      edge => (
        <div className="col-3">
    ...
    const listSponsorsAssociate = this.props.data.sponsorsAssociateLogos.edges.map(
      edge => (
        <div className="col-3">
          <Img fluid={edge.node.childImageSharp.fluid} />
        </div>
    ...
    const listPartenaires = this.props.data.partenairesLogos.edges.map(edge => (
      <div className="col-3">
        <Img fluid={edge.node.childImageSharp.fluid} />
      </div>
    ));
...
export const pageQuery = graphql`
  query SponsorsQuery {
    ...
    sponsorsProfessionalLogos: allFile(
      filter: {
        extension: { regex: "/(png|jpe?g)/" }
        relativePath: { regex: "/sponsors/professional/" }
      }
    ) {
      edges {
        node {
    ...
    sponsorsAssociateLogos: allFile(
      filter: {
        extension: { regex: "/(png|jpe?g)/" }
        relativePath: { regex: "/sponsors/associate/" }
      }
    ) {
      edges {
        node {
    ...
    partenairesLogos: allFile(
      filter: {
        extension: { regex: "/(png|jpe?g)/" }
        relativePath: { regex: "/sponsors/partenaires/" }
      }
    ) {
      edges {
        node {
